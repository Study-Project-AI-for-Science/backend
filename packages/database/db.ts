...
export async function paperInsert(
  ...
  try {
    ...
    const existingPaper = await useDrizzle()
      .select({ id: papers.id })
      .from(papers)
    ...
    if (existingPaper.length > 0) {
      console.log(
        `Paper at ${filePath} already exists with ID: ${existingPaper[0]!.id}. Skipping insertion.`,
      )
    ...
    if (
      ...
      !insertedPaperResult[0]?.insertedId
    ) {
    ...
    try {
      ...
      const modelName = embeddingInfo.model_name || undefined
    ...
    await db.transaction(async (tx) => {
      ...
      if (referencesToUpdate.length > 0) {
        await tx
          ...
          .where(eq(paperReferences.referencePaperId, paperId))
        console.log(
          `Nullified ${referencesToUpdate.length} references pointing to paper ${paperId}`,
        )
      } else {
...
export async function processReferenceWithArxivId(
  ...
  try {
    ...
    // Download the paper
    const downloadedFilePath = await paperDownloadArxivId(arxivId, tempDir) // Assumes this function downloads and returns the path
