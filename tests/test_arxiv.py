...
def test_paper_download_arxiv_id_success(mock_arxiv):
    ...
    mock_arxiv.query.assert_called_once_with(id_list=[TEST_ARXIV_ID])

def test_paper_download_arxiv_id_not_found(mock_arxiv):
    """Test paper download with non-existent arXiv ID"""
    mock_arxiv.query.return_value = iter([])
    with pytest.raises(ArxivPaperNotFoundError):
        paper_download_arxiv_id(TEST_ARXIV_ID, TEST_OUTPUT_DIR)

def test_paper_download_arxiv_id_download_error(mock_arxiv):
    ...
    mock_arxiv.query.return_value = iter([paper])
    
    with pytest.raises(ArxivDownloadError):
        paper_download_arxiv_id(TEST_ARXIV_ID, TEST_OUTPUT_DIR)

def test_paper_download_arxiv_metadata_success(mock_arxiv):
    ...
    assert result == TEST_PDF_PATH
    mock_arxiv.query.assert_called_once()

def test_paper_download_arxiv_metadata_not_found(mock_arxiv):
    """Test paper download with non-matching metadata"""
    mock_arxiv.query.return_value = iter([])
    with pytest.raises(ArxivPaperNotFoundError):
        paper_download_arxiv_metadata(TEST_AUTHORS, TEST_TITLE, TEST_OUTPUT_DIR)

def test_paper_get_metadata_from_filename(mock_arxiv):
...
def test_paper_get_metadata_from_pdf_content(mock_arxiv, mock_pymupdf):
    ...
    assert mock_arxiv.query.call_count == 2
    mock_pymupdf.open.assert_called_once_with("test_paper.pdf")

def test_paper_get_metadata_not_found(mock_arxiv, mock_pymupdf):
