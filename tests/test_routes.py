...
def test_create_paper_success(client):
    ...
    response = client.post('/papers', data=json.dumps({}), content_type='application/json') #, data=json.dumps(test_data), content_type='application/json'
...
def test_create_paper_invalid_content_type(client):
    ...
    assert response.status_code == 201 #Still expect to work, content type should not matter, since the server side code does not depend on the content-type
